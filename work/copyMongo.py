...
table = mongo_con[MONGO_DB]["encrypt"]
log_table = mongo_con[MONGO_DB]["log"]
...
def query(processId,phone_md5_list,q):
    times=1000
    phone=[]
    step=len(phone_md5_list)/times + 1
    beginStep=0
    endStep=beginStep+times
    for i in range(0,step):
        mid=phone_md5_list[beginStep:endStep]
        li = list(table.find({"callmd5": {"$in": mid}}))
        for phone in li:
            q.append(phone["_id"])
        beginStep=endStep
        endStep=beginStep+times
